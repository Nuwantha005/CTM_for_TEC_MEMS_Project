## TEC Design

1) [x] Basic Research about [[TEC]]
2) [x] Finding Governing Equations for the Optimization Process
3) [ ] [[Basic Radial TEC Design]]
	1) [ ] [[TEC Thermal Network]]
	2) [x] Geometric Arrangement
	3) [x] Electric Network
	4) [x] Basic Geometric Arrangement
	5) [x] [[Variable Cross section area TEC]]
4) [ ] Formulating the optimization problem
	1) [ ] Solver
	2) [ ] [[Resistance Network Solver and COMSOL Result Comparison]]
	3) [ ] [[Radial TEC Parameter List]]
	4) [ ] Objective Function
	5) [ ] [[Radial TEC Constraints]]
5) [ ] [[Preliminary Optimization of TEC]]
	1) [ ] Plots
	2) [ ] Comparison of different modes
6) [ ] Basic Design on COMSOL for selected candidates
7) [ ] [[Advanced Optimization of selected TEC]]
8) [ ] Mechanical Analysis
	1) [ ] Structural Analysis - Stress, Strain
	2) [ ] Thermal TSVs
	3) [ ] Thermal Stresses (Expansion/Contraction)
	4) [ ] Modal Analysis
9) [ ] Modifications
10) [ ] Re-Iterate

>[!Advanced Optimization Process]
>$$\begin{matrix}\mathrm{Parametric}&& && \mathrm{COMSOL}\\\mathrm{SolidWorks}&&\longleftrightarrow&&\updownarrow\\\mathrm{model}&&&&\mathrm{MATLAB}\end{matrix}$$

## Micro-channel Design

1) [ ] Basic Dimensions preliminary Study
2) [ ] Heat Transfer Characteristics in Circular Domain
3) [ ] Basic Structure Simulations in Ansys / COMSOL
4) [ ] Flow-path Optimization
5) [ ] Wall Modifications
6) [ ] Coupled Simulations
7) [ ] Advanced Optimization
8) [ ] Mechanical Analysis
	1) [ ] Structural Analysis - Stress, Strain
	2) [ ] Thermal Stresses (Expansion/Contraction)
	3) [ ] Modal Analysis
9) [ ] Modifications
10) [ ] Re-Iterate

# Material Selection
1) [ ] Search and find most common materials
2) [ ] Run Preliminary optimization with each material and filter.
3) [ ] Run full simulation with materials and filter
4) [ ] Compare with fabrication methods and filter out
5) [ ] Final Selection and finishing
# Fabrication Method

1) [ ] Refer existing literature - [[TEC Fabrication Techniques]]
2) [ ] Select the most suitable method
3) [ ] Plan fabrication process
4) [ ] Give mask designs and layer designs
	1) [ ] Curved Geometries OK ? (Curved Masks ?)
5) [ ] Packaging
6) [ ] Final step by step fabrication process
