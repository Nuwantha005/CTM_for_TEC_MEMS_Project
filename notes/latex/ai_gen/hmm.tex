...
The central region $0 \le r \le R_{\text{cyl}}$ is modelled as node $T_0$, with internal thermal resistance $R_{\text{int,chip}}$ and volumetric heat generation $Q_{\text{gen},0}$ as described in Section~\ref{sec:radial_wedge_geometry}.
...
\begin{itemize}
	...
	\item $\theta$: Azimuthal span of both TE legs
...
The copper p--n interconnect (yellow, black arrow) links the p and n legs in the same TEC element. Its radial length and thickness are denoted $w_{\text{ic},i}$ and $t_{\text{ic},i}$, and its azimuthal span by $\beta_{\text{ic},i}$. These parameters control both the vertical resistance to TSVs (through the contact area and TSV count) and the in-plane electrical resistance $R_{e,\text{ic}}$.
...
Radial insulators of width $w_{\text{is}}$ separate successive TEC stages to suppress direct conduction while still allowing radial conduction. Their radial resistance $R_{\text{is}}$ is given by Eq.~\eqref{eq:R_is}. This resistance is added in series to the TEC-leg resistance $1/K_{\text{total,TE}}$ to form $R_{\text{eff,series}}$.
...
\begin{align}
	...
	\label{eq:R_TE_i}\\
	K_{\text{total,TE},i} &= \frac{1}{R_{k,p,i}} + \frac{1}{R_{k,n,i}} = \frac{k_p + k_n}{G_i},
...
\label{sec:radial_wedge_geometry}
...
The chip layer also hosts volumetric power dissipation. We model this as a uniform heat flux $q''$ [W/m$^2$] applied over the top surface of each radial segment. For a segment spanning radii $r_{i-1}$ to $r_i$, the top area is the sector area
\begin{equation}
...
\begin{equation}
	 Q_{\text{gen},0} = q''_{\text{flux}} \, \frac{1}{2} R_{\text{cyl}}^2 \, \theta,
	\label{eq:Qgen0}
\end{equation}
where $q''_{\text{flux}}$ denotes the chip-side heat flux for the \emph{full} device and the factor $\theta$ accounts for the fact that a single wedge occupies only a fraction $\theta/(2\pi)$ of the total circumference. Equivalently, one may prescribe a total chip power $Q_{\text{tot}}$ and scale it to the representative wedge as $Q_{\text{gen},0} = Q_{\text{tot}}/N_\theta$.
...
\begin{align}
	Q_{h,i-1} &= S_{i-1} I_{i-1} T_{c,i}
		+ \tfrac{1}{2} I_{i-1}^2 R_{e,i-1}
		- K_{i-1} (T_{c,i} - T_{c,i-1}),
		\label{eq:Qh_i_minus1} \\
	Q_{c,i} &= S_i I_i T_{c,i}
		- \tfrac{1}{2} I_i^2 R_{e,i}
		+ K_i (T_{c,i+1} - T_{c,i}).
		\label{eq:Qc_i}
\end{align}
...
\begin{multline}
	...
	- \left( \frac{1}{R_{v,i}} + K_{i-1} - S_{i-1} I_{i-1} + S_i I_i + K_i \right) T_{c,i} \\
	+ K_i T_{c,i+1}
	+ \left( \frac{1}{R_{v,i}} \right) T_{\text{Si},i}
	= - \frac{1}{2} I_{i-1}^2 R_{e,i-1} - \frac{1}{2} I_i^2 R_{e,i}.
	\label{eq:tec_coeff}
\end{multline}

At the outermost TEC node $i=N$, the final stage rejects heat to the water boundary, which is imposed by eliminating $T_{c,N+1}$ in favour of $T_{\text{w}}$ using $T_{c,N+1} = T_{\text{w}}$ in Eq.~\eqref{eq:Qc_i}, thereby modifying the diagonal of $A_{\text{TEC}}$ and the corresponding entry in $B_{\text{TEC}}$ by a term $-K_N T_{\text{w}}$.
...
\begin{equation}
	R_{\text{TEC},0\to1} = \frac{1}{t \theta} \left[ \frac{1}{2 k_{\text{Si}}} + \frac{1}{k_{\text{is}}}
		\ln\!\left( \frac{R_{\text{cyl}}}{R_{\text{cyl}} + w_{\text{is}}} \right) \right].
	\label{eq:R_TEC_0_1}
\end{equation}
...
where
\begin{itemize}
	\item $M_{00} = -\left( \frac{1}{R_{\text{Si},0\to1}} + \frac{1}{R_{\text{TEC},0\to1}} \right)$,
...
where
\begin{equation}
	b_{\text{TEC},i} = -\tfrac{1}{2} I_{i-1}^2 R_{e,i-1} - \tfrac{1}{2} I_i^2 R_{e,i} + b_{\text{bc},i},
	\label{eq:b_tec_i}
\end{equation}
