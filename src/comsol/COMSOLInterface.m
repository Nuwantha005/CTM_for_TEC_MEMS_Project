classdef COMSOLInterface < handle
    ...
    properties (Constant)
        ...
        % Updated for paper notation (Thermal_Network_For_Radial_TEC.tex)
        % TSV parameters have been REMOVED from the model
        ParamMap = struct(...
            'theta_deg', 'LL_theta', ...       % θ: Wedge angle (derived from M)
            'M_wedges', 'LL_M', ...            % M: Number of wedges (integer)
            't_TEC_um', 'LL_t_TEC', ...        % t_TEC: TEC layer thickness
            't_chip_um', 'LL_t_chip', ...      % t_chip: Chip thickness
            't_ins_um', 'LL_t_ins', ...        % t_ins: Insulator layer thickness (NEW)
            'r_cyl_um', 'LL_R_cyl', ...        % r_cyl: Central cylinder radius
            'L_1_um', 'LL_L_1', ...            % L_1: First stage length
            'f_L', 'LL_k_r', ...               % f_L: Length ratio (was k_r)
            'W_ic_um', 'LL_w_ic', ...          % W_ic: Interconnector width
            'W_oc_um', 'LL_w_oc', ...          % W_oc: Outerconnector width
            't_ic_um', 'LL_t_ic', ...          % t_ic: Interconnector thickness
            't_oc_um', 'LL_t_oc', ...          % t_oc: Outerconnector thickness
            'W_is_um', 'LL_w_is', ...          % W_is: Radial insulator width
            'W_az_um', 'LL_w_az', ...          % W_az: Azimuthal insulator width
            'beta_ic_deg', 'LL_beta_ic', ...   % β_ic: Interconnector angle
            'beta_oc_deg', 'LL_beta_oc', ...   % β_oc: Outerconnector angle
            'r_chip_mm', 'LL_r_chip', ...      % r_chip: Chip radius
            'I_current_A', 'I0', ...           % I: Operating current
            'q_flux_W_m2', 'q' ...             % q: Heat flux
            );
    end
